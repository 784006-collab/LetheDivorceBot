...
async def on_message(message):
    if "cp" in message.content.lower() or (
             "old lungs" in message.content.lower() and message.author.id in [1375130091725783150]):
         try:
    ...
    if "ping em john" in message.content.lower():
        ...
        user_id = 465275102326685697  # John’s Discord ID
        user = await message.guild.fetch_member(user_id)  # fetch user from guild

        if user:  # make sure user is found
            await message.channel.send(f"{user.mention} ping em john")
        else:

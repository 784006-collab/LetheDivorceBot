...
class DungeonGenerator:
    ...
    def generate(self):
        ...
        for d in range(1, self.depth + 1):
            ...
            for l in range(self.num_lanes):
                r = random.random()
                if r < 0.15 and GENERATE_EVENTS:
                    node_type = "event"
                elif r < 0.30 and GENERATE_HARD:  # 0.15–0.30
                    node_type = "hard"
                elif GENERATE_NORMAL:  # 0.30–1.0
                    node_type = "normal"
                elif GENERATE_HARD:
                    node_type = "hard"
    ...
    def visualize_grid_vertical(self, dungeon):
        height = self.depth + 3
        ...
        for node_id, data in dungeon["nodes"].items():
            ...
            col = lane if lane is not None else width // 2
            row = depth
            ...
            bboxes[node_id] = (start_x, y, start_x + len(label) - 1, y)

        # Draw connections (individually routed, no shared bus)
        # Draw connections (shorter horizontal spans, no shared bars)
        for a, b in dungeon["connections"]:
            x1, y1 = positions[a]
            x2, y2 = positions[b]

            # normalize: always draw top→bottom
            if y1 > y2:
            ...
            if x1 == x2:
                for y in range(y1 + 1, y2):
                    if not any(x1b <= x1 <= x2b and y1b <= y <= y2b for (x1b, y1b, x2b, y2b) in bboxes.values()):
                        grid[y][x1] = self.merge_char(grid[y][x1], "│")
                grid[y2][x2] = "▼"
                continue

            # case 2: needs horizontal offset
            mid_y = (y1 + y2) // 2

            # vertical down to elbow
            for y in range(y1 + 1, mid_y):
                if not any(x1b <= x1 <= x2b and y1b <= y <= y2b for (x1b, y1b, x2b, y2b) in bboxes.values()):
                    grid[y][x1] = self.merge_char(grid[y][x1], "│")

            # elbow
            grid[mid_y][x1] = "└" if x2 > x1 else "┘"

            # horizontal between x1 and x2 (short!)
            step = 1 if x2 > x1 else -1
            for x in range(x1 + step, x2, step):
                if not any(x1b <= x <= x2b and y1b <= mid_y <= y2b for (x1b, y1b, x2b, y2b) in bboxes.values()):
                    grid[mid_y][x] = self.merge_char(grid[mid_y][x], "─")

            # elbow at x2
            grid[mid_y][x2] = "┐" if x2 > x1 else "┌"

            # vertical down from elbow to target
            for y in range(mid_y + 1, y2):
                if not any(x1b <= x2 <= x2b and y1b <= y <= y2b for (x1b, y1b, x2b, y2b) in bboxes.values()):
                    grid[y][x2] = self.merge_char(grid[y][x2], "│")
...
if __name__ == "__main__":
    #     generator = DungeonGenerator(num_lanes=4, depth=3, seed=2977298)
    generator = DungeonGenerator(num_lanes=4, depth=3, seed=1023)

...
class SetOffenseLevelHandler(ModifierHandler):
    ...
    async def apply(self, value, modifier_target : ProfileData, acquired_values, effect, log, roll_container, pagename, symbol, **kwargs):
        ...
        olBlock = value
        source = olBlock.get("source")
        # modifier_value = olBlock.get("modifier")
        modifier_value = olBlock.get("value", 0)
        ...
        print(f"⚡ {modifier_target.name} sets offense level source '{source}' to {resolved_value}"
)

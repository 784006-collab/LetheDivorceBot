    ...
    async def apply(self, value, modifier_target : ProfileData, acquired_values, effect, log, roll_container, pagename, symbol, **kwargs):
        ...
        for target_die in targets:
            ...
            if "boostmin" in changedice:
                boostmin = resolve_value(changedice.get("boostmin"), acquired_values)
                target_die["min"] = target_die.get("min", 0) + boostmin
                
            if "boostmax" in changedice:
                boostmax = resolve_value(changedice.get("boostmax"), acquired_values)
                target_die["max"] = target_die.get("max", 0) + boostmax

            if "forcetype" in changedice:
                target_die["type"] = changedice.get("forcetype")
